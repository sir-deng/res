...
class p implements q {
    ...
    public void eF() {
        af.Ws = new a() {
            public final void a(Canvas canvas, RectF rectF, float f, Paint paint) {
                ...
                if (f >= 1.0f) {
                    float f3 = f + 0.5f;
                    p.this.Qy.set(-f3, -f3, f3, f3);
                    int save = canvas.save();
                    canvas.translate(rectF.left + f3, rectF.top + f3);
                    canvas.drawArc(p.this.Qy, 180.0f, 90.0f, true, paint);
    ...
    public final float b(o oVar) {
        af d = d(oVar);
        return ((((float) d.Wr) + (d.ji * 1.5f)) * 2.0f) + (Math.max(d.ji, (d.jf + ((float) d.Wr)) + ((d.ji * 1.5f) / 2.0f)) * 2.0f);
    }
