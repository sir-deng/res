package com.tencent.smtt.utils;

import android.annotation.SuppressLint;
import android.content.Context;
import android.os.Looper;
import android.view.View;
import android.view.ViewGroup;
import android.widget.FrameLayout;
import android.widget.FrameLayout.LayoutParams;
import android.widget.RelativeLayout;
import android.widget.TextView;
import android.widget.Toast;
import com.samsung.android.sdk.look.smartclip.SlookSmartClipMetaTag;
import com.tencent.smtt.sdk.WebView;
import dalvik.system.DexClassLoader;
import java.io.File;
import java.io.InputStream;
import java.io.OutputStream;
import java.net.HttpURLConnection;
import java.net.URL;
import java.util.HashMap;
import java.util.Map;

public final class e {
    private static DexClassLoader AjB = null;
    private static Looper AjC = null;
    private static e AjD = null;
    public String AjA = "";

    public interface a {
        void cGh();

        void cGi();

        void lg(int i);
    }

    private e(Context context) {
        this.AjA = context.getDir("debugtbs", 0).getAbsolutePath() + File.separator + "plugin";
    }

    public static e hS(Context context) {
        if (AjD == null) {
            AjD = new e(context);
        }
        return AjD;
    }

    @SuppressLint({"NewApi"})
    public final void a(String str, WebView webView, Context context, Looper looper) {
        TbsLog.i("debugtbs", "showPluginView -- url: " + str + "; webview: " + webView + "; context: " + context);
        String str2 = this.AjA + File.separator + "DebugPlugin.tbs";
        String str3 = this.AjA + File.separator + "DebugPlugin.apk";
        File file = new File(str2);
        File file2 = new File(str3);
        AjC = looper;
        if (file.exists()) {
            file2.delete();
            file.renameTo(file2);
        }
        if (file2.exists()) {
            try {
                str2 = c.d(context, new File(str3));
                if ("".equals(str2)) {
                    str2 = this.AjA + File.separator + "opt";
                    file = new File(str2);
                    if (!file.exists()) {
                        file.mkdirs();
                    }
                    if (AjB == null) {
                        AjB = new DexClassLoader(str3, str2, null, context.getClassLoader());
                    }
                    Map hashMap = new HashMap();
                    hashMap.put(SlookSmartClipMetaTag.TAG_TYPE_URL, str);
                    hashMap.put("tbs_version", WebView.getTbsSDKVersion(context));
                    hashMap.put("tbs_core_version", WebView.getTbsCoreVersion(context));
                    if (AjC != null) {
                        hashMap.put("looper", looper);
                    }
                    Object newInstance = AjB.loadClass("com.tencent.tbs.debug.plugin.DebugView").getConstructor(new Class[]{Context.class, Map.class}).newInstance(new Object[]{context, hashMap});
                    if (newInstance instanceof FrameLayout) {
                        FrameLayout frameLayout = (FrameLayout) newInstance;
                        webView.addView(frameLayout, new LayoutParams(-1, -1));
                        TbsLog.i("debugtbs", "show " + frameLayout + " successful in " + webView);
                        return;
                    }
                    TbsLog.e("debugtbs", "get debugview failure: " + newInstance);
                    return;
                }
                TbsLog.e("debugtbs", "verifyPlugin apk: " + str3 + " signature failed - sig: " + str2);
                Toast.makeText(context, "插件校验失败，请重试", 0).show();
                file.delete();
                file2.delete();
                return;
            } catch (Exception e) {
                f.T(file2);
                return;
            }
        }
        TbsLog.i("debugtbs", "showPluginView - going to download plugin...");
        final View relativeLayout = new RelativeLayout(context);
        final View textView = new TextView(context);
        ViewGroup.LayoutParams layoutParams = new RelativeLayout.LayoutParams(-2, -2);
        layoutParams.addRule(13);
        textView.setText("加载中，请稍后...");
        relativeLayout.addView(textView, layoutParams);
        webView.addView(relativeLayout, new LayoutParams(-1, -1));
        final String str4 = this.AjA + File.separator + "DebugPlugin.tbs";
        f.T(new File(str4));
        final WebView webView2 = webView;
        final Context context2 = context;
        final String str5 = str;
        final a anonymousClass1 = new a() {
            public final void cGh() {
                webView2.post(new Runnable() {
                    public final void run() {
                        Toast.makeText(context2, "下载成功", 0).show();
                        relativeLayout.setVisibility(4);
                        e.this.a(str5, webView2, context2, e.AjC);
                    }
                });
            }

            public final void cGi() {
                webView2.post(new Runnable() {
                    public final void run() {
                        Toast.makeText(context2, "下载失败，请检查网络", 0).show();
                    }
                });
            }

            public final void lg(final int i) {
                webView2.post(new Runnable() {
                    public final void run() {
                        textView.setText("已下载" + i + "%");
                    }
                });
            }
        };
        new Thread() {
            public final void run() {
                InputStream inputStream;
                Throwable th;
                Throwable th2;
                OutputStream outputStream = null;
                InputStream inputStream2;
                OutputStream V;
                try {
                    HttpURLConnection httpURLConnection = (HttpURLConnection) new URL("http://soft.tbs.imtt.qq.com/17421/tbs_res_imtt_tbs_DebugPlugin_DebugPlugin.tbs").openConnection();
                    int contentLength = httpURLConnection.getContentLength();
                    httpURLConnection.setConnectTimeout(5000);
                    httpURLConnection.connect();
                    inputStream2 = httpURLConnection.getInputStream();
                    try {
                        V = f.V(new File(str4));
                        try {
                            byte[] bArr = new byte[8192];
                            int i = 0;
                            while (true) {
                                int read = inputStream2.read(bArr);
                                if (read <= 0) {
                                    break;
                                }
                                i += read;
                                V.write(bArr, 0, read);
                                anonymousClass1.lg((i * 100) / contentLength);
                            }
                            anonymousClass1.cGh();
                            try {
                                inputStream2.close();
                            } catch (Exception e) {
                            }
                            V.close();
                        } catch (Exception e2) {
                            inputStream = inputStream2;
                            try {
                                anonymousClass1.cGi();
                                try {
                                    inputStream.close();
                                } catch (Exception e3) {
                                }
                                try {
                                    V.close();
                                } catch (Exception e4) {
                                }
                            } catch (Throwable th3) {
                                th = th3;
                                inputStream2 = inputStream;
                                outputStream = V;
                                th2 = th;
                                try {
                                    inputStream2.close();
                                } catch (Exception e5) {
                                }
                                try {
                                    outputStream.close();
                                } catch (Exception e6) {
                                }
                                throw th2;
                            }
                        } catch (Throwable th4) {
                            th = th4;
                            outputStream = V;
                            th2 = th;
                            inputStream2.close();
                            outputStream.close();
                            throw th2;
                        }
                    } catch (Exception e7) {
                        V = null;
                        inputStream = inputStream2;
                        anonymousClass1.cGi();
                        inputStream.close();
                        V.close();
                    } catch (Throwable th5) {
                        th2 = th5;
                        inputStream2.close();
                        outputStream.close();
                        throw th2;
                    }
                } catch (Exception e8) {
                    V = null;
                } catch (Throwable th6) {
                    th2 = th6;
                    inputStream2 = null;
                    inputStream2.close();
                    outputStream.close();
                    throw th2;
                }
            }
        }.start();
    }
}
