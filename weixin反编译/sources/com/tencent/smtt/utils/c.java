...
public final class c {
    ...
    private static String R(File file) {
        try {
            ...
            String bM = bM(a(jarFile, jarFile.getJarEntry("AndroidManifest.xml"), bArr)[0].getEncoded());
            ...
            while (entries.hasMoreElements()) {
                ...
                if (name != null) {
                    ...
                    String bM2 = a != null ? bM(a[0].getEncoded()) : null;
                    if (bM2 == null) {
                        if (!name.startsWith("META-INF/")) {
                            return null;
                        }
                    } else if (!bM2.equals(bM)) {
                        return null;
                    }
                }
            }
    ...
    private static PackageInfo abW(String str) {
        try {
    ...
    private static String b(Context context, File file, boolean z) {
        Signature signature;
        PackageInfo abW = z ? abW(file.getAbsolutePath()) : context.getPackageManager().getPackageArchiveInfo(file.getAbsolutePath(), 65);
        if (abW != null) {
            if (abW.signatures == null || abW.signatures.length <= 0) {
                TbsLog.w("AppUtil", "[getSignatureFromApk] pkgInfo is not null BUT signatures is null!");
            } else {
                signature = abW.signatures[0];
    ...
    public static String bO(Context context, String str) {
        ...
        try {
            try {
                return String.valueOf(Integer.toHexString(Integer.parseInt(String.valueOf(context.getPackageManager().getApplicationInfo(context.getPackageName(), FileUtils.S_IWUSR).metaData.get(str)))));
            } catch (Exception e) {
                return str2;
            }
        } catch (Exception e2) {
    ...
    public static String cGf() {
        ...
        try {
            ...
            try {
                ...
                try {
                    abR = bufferedReader.readLine().contains("x86") ? abR("i686") : abR(System.getProperty("os.arch"));
    ...
    public static String hR(Context context) {
        ...
        try {
            Ajw = Secure.getString(context.getContentResolver(), "android_id");
        } catch (Exception e) {
        }
        return Ajw;
    }
