...
public class RecoveryUploadService extends IntentService {
    ...
    public static synchronized void startAlarm(Context context, String str) {
        synchronized (RecoveryUploadService.class) {
            if (!Util.oN(str)) {
                CommonOptions gk = OptionFactory.gk(context);
                ...
                RecoveryLog.i(TAG, "startAlarm %s lastUploadTime %s", str, Util.gu(j));
                ...
                if (service == null || currentTimeMillis - j > gk.AaS) {
                    if (service != null) {
                        service.cancel();
                    ...
                    try {
                        ((AlarmManager) context.getSystemService("alarm")).set(0, (gk.AaS + currentTimeMillis) - 2000, PendingIntent.getService(context, 1000001, intent, 134217728));
                    } catch (Throwable e) {
                        RecoveryLog.printErrStackTrace(TAG, e, "alarmManager.setAlarm", new Object[0]);
                    }
                    RecoveryLog.i(TAG, "startAlarm pendingIntent success: %d will report %s", Integer.valueOf(r2.hashCode()), Util.gu(j));
                } else {
    ...
    public static void startAlarmAfter(Context context, String str, long j) {
        if (!Util.oN(str)) {
            ...
            RecoveryLog.i(TAG, "startAlarmAfter pendingIntent:%d %s", Integer.valueOf(service.hashCode()), Util.gu(r2));
        }
    }
