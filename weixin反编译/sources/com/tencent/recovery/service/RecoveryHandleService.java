...
public class RecoveryHandleService extends IntentService {
    private static volatile boolean fmm;
    ...
    protected final void onHandleIntent(Intent intent) {
        ...
        if (recoveryData == null || recoveryData.processName == null) {
            RecoveryLog.i("Recovery.RecoveryHandleService", "recoveryData is null %b %d %s %s", Boolean.valueOf(booleanExtra), Integer.valueOf(gm), stringExtra, stringExtra2);
            return;
        }
        ...
        if (fmm) {
            ...
            RecoveryLog.i("Recovery.RecoveryHandleService", "start to handle %s's exception uuid: %s threadId: %d", recoveryData.processName, recoveryData.njL, Long.valueOf(Thread.currentThread().getId()));
            ...
            if (recoveryHandleResult.AaO && !booleanExtra) {
                ...
                long currentTimeMillis = (System.currentTimeMillis() + gk.AaT) - 2000;
                ((AlarmManager) getSystemService("alarm")).set(0, currentTimeMillis, PendingIntent.getService(this, 1000002, intent3, 0));
                RecoveryLog.i("Recovery.RecoveryHandleService", "startAlarm pendingIntent success: %d will retry %s", Integer.valueOf(r2.hashCode()), Util.gu(currentTimeMillis));
            }
