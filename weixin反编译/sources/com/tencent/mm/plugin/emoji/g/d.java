            ...
            for (int i = 0; i < elementsByTagName.getLength(); i++) {
                ...
                for (int i2 = 0; i2 < length; i2++) {
                    ...
                    if (item != null && item.getNodeName() != null && item.getNodeName().equals("ProductID")) {
                        x.d("MicroMsg.emoji.EmojiRecommendXMLParser", "[recommend emotion parser] productId:%s", item.getTextContent());
                        ...
                        if (item != null) {
                            if (item.getNodeName() != null && item.getNodeName().equals("RecWord")) {
                                x.d("MicroMsg.emoji.EmojiRecommendXMLParser", "[recommend emotion parser] recWord:%s", item.getTextContent());
                                emojiGroupInfo.field_recommandWord = r9;
                            }
                        }
                        if (item != null && item.getNodeName() != null && item.getNodeName().equals("Name")) {
                            x.d("MicroMsg.emoji.EmojiRecommendXMLParser", "[recommend emotion parser] Name:%s", item.getTextContent());
                            emojiGroupInfo.field_packName = r9;
                        } else if (item != null && item.getNodeName() != null && item.getNodeName().equals("Panel")) {
                            x.d("MicroMsg.emoji.EmojiRecommendXMLParser", "[recommend emotion parser] iconUrl:%s", item.getTextContent());
                            emojiGroupInfo.field_packGrayIconUrl = r9;
                        } else if (item != null && item.getNodeName() != null && item.getNodeName().equals("Icon")) {
                            x.d("MicroMsg.emoji.EmojiRecommendXMLParser", "[recommend emotion parser] BigIconUrl:%s", item.getTextContent());
                            emojiGroupInfo.field_BigIconUrl = r9;
                        } else if (!(item == null || item.getNodeName() == null || !item.getNodeName().equals("ButtonType"))) {
                            x.d("MicroMsg.emoji.EmojiRecommendXMLParser", "[recommend emotion parser] buttonType:%s", item.getTextContent());
                            emojiGroupInfo.field_buttonType = Integer.valueOf(r9).intValue();
                        }
                    } else {
                        x.d("MicroMsg.emoji.EmojiRecommendXMLParser", "[recommend emotion parser] RecType:%s", item.getTextContent());
                        emojiGroupInfo.field_recommandType = Integer.valueOf(r9).intValue();
                    }
                }
                if (!bi.oN(emojiGroupInfo.field_productID)) {
                    emojiGroupInfo.field_recommand = 1;
                    emojiGroupInfo.field_lastUseTime = System.currentTimeMillis();
                    emojiGroupInfo.field_sort = 0;
                    g.pWK.h(12068, Integer.valueOf(1), emojiGroupInfo.field_productID, Integer.valueOf(emojiGroupInfo.field_recommandType));
                    arrayList.add(emojiGroupInfo);
                }
            }
            return arrayList;
        } catch (Exception e) {
            x.e("MicroMsg.emoji.EmojiRecommendXMLParser", "[parser] parseXML exception:%s", e.toString());
            return null;
        }
    }
}
