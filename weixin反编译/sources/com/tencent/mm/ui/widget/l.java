...
public final class l {
    private static LinkedList<WeakReference<a>> zFr = new LinkedList();
    ...
    public static void a(a aVar) {
        x.d("MicroMsg.SwipeBackHelper", "pushCallback size %d, %s", Integer.valueOf(zFr.size()), aVar);
        zFr.add(0, new WeakReference(aVar));
    }

    public static boolean b(a aVar) {
        x.d("MicroMsg.SwipeBackHelper", "popCallback size %d, %s", Integer.valueOf(zFr.size()), aVar);
        if (aVar == null) {
            return true;
        }
        LinkedList linkedList = new LinkedList();
        for (int i = 0; i < zFr.size(); i++) {
            if (aVar == ((WeakReference) zFr.get(i)).get()) {
                zFr.remove(i);
        ...
        if (aVar.forceRemoveNoMatchOnPath() || linkedList.size() != r4) {
            ...
            while (it.hasNext()) {
                WeakReference weakReference = (WeakReference) zFr.remove(((Integer) it.next()).intValue());
    ...
    public static void aI(float f) {
        ...
        a aVar = (a) ((WeakReference) zFr.get(0)).get();
        if (aVar == null) {
