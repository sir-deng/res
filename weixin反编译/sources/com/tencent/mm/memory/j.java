...
public final class j extends l {
    private Bitmap a(InputStream inputStream, Options options, DecodeResultLogger decodeResultLogger) {
        ...
        Bitmap bitmap = null;
        try {
            ByteBuffer byteBuffer2 = (ByteBuffer) h.hbJ.bH();
            try {
            ...
            obj = bitmap;
            try {
                x.e("MicroMsg.InBitmapFactory", "error decode bitmap %s", e.getMessage());
                if (obj == null) {
                    return bitmap;
                }
                h.hbJ.j(obj);
    ...
    public final Bitmap a(String str, Options options, DecodeResultLogger decodeResultLogger) {
        ...
        try {
            ...
            try {
                ...
                e = e2;
                try {
                    x.printErrStackTrace("MicroMsg.InBitmapFactory", e, "error decode bitmap %s", e.getMessage());
                    e.c(openRead);
                    return bitmap;
                } catch (Throwable th2) {
                    th = th2;
    ...
    private static Options a(InputStream inputStream, Options options, ByteBuffer byteBuffer, boolean z, Rect rect) {
        ...
        if (rect == null) {
            x.i("MicroMsg.InBitmapFactory", "options.outWidth: %s, options.outHeight: %s, sampleSize: %s", Integer.valueOf(i2), Integer.valueOf(i3), Integer.valueOf(options.inSampleSize));
            i = i2;
            i2 = i3;
        } else {
            x.i("MicroMsg.InBitmapFactory", "region decode, width: %s, height: %s, rect: %s", Integer.valueOf(rect.right - rect.left), Integer.valueOf(rect.bottom - rect.top), rect);
            i = i2;
            i2 = i3;
        }
        c cVar;
        ...
        x.i("MicroMsg.InBitmapFactory", "mimetype: %s", options.outMimeType);
        return options;
    }
            ...
            if (!(rect == null || decodeStream == null)) {
                try {
                    if (decodeStream.getWidth() > rect.right - rect.left && decodeStream.getHeight() > rect.bottom - rect.top) {
                        x.i("MicroMsg.InBitmapFactory", "fuck region decode size not match, clip it, reqWidth: %s, reqHeight: %s, resultWidth: %s, resultHeight: %s", Integer.valueOf(rect.right - rect.left), Integer.valueOf(rect.bottom - rect.top), Integer.valueOf(decodeStream.getWidth()), Integer.valueOf(decodeStream.getHeight()));
                    ...
                    bitmap = decodeStream;
                    e = exception;
                    x.e("MicroMsg.InBitmapFactory", "decodeInternal error: %s", e.getMessage());
                    decodeStream = bitmap;
                    if (decodeStream != null) {
                        x.i("MicroMsg.InBitmapFactory", "finish decode, decodedBitmap: %s, width: %s, height: %s", decodeStream, Integer.valueOf(decodeStream.getWidth()), Integer.valueOf(decodeStream.getHeight()));
                    }
                    return decodeStream;
                }
            }
