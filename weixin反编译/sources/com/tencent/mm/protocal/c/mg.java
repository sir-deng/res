package com.tencent.mm.protocal.c;

import e.a.a.b;
import e.a.a.c.a;
import java.util.LinkedList;

public final class mg extends bea {
    public String fxT;
    public String pQZ;
    public String pRd;
    public long pSo;
    public String wbF;
    public String wbo;

    protected final int a(int i, Object... objArr) {
        int fW;
        if (i == 0) {
            a aVar = (a) objArr[0];
            if (this.fxT == null) {
                throw new b("Not all required fields were included: req_key");
            } else if (this.wbF == null) {
                throw new b("Not all required fields were included: transfer_id");
            } else if (this.pRd == null) {
                throw new b("Not all required fields were included: transaction_id");
            } else if (this.pQZ == null) {
                throw new b("Not all required fields were included: receiver_openid");
            } else {
                if (this.wQE != null) {
                    aVar.fZ(1, this.wQE.bkL());
                    this.wQE.a(aVar);
                }
                if (this.fxT != null) {
                    aVar.g(2, this.fxT);
                }
                if (this.wbF != null) {
                    aVar.g(3, this.wbF);
                }
                if (this.pRd != null) {
                    aVar.g(4, this.pRd);
                }
                if (this.pQZ != null) {
                    aVar.g(5, this.pQZ);
                }
                aVar.S(6, this.pSo);
                if (this.wbo == null) {
                    return 0;
                }
                aVar.g(7, this.wbo);
                return 0;
            }
        } else if (i == 1) {
            if (this.wQE != null) {
                fW = e.a.a.a.fW(1, this.wQE.bkL()) + 0;
            } else {
                fW = 0;
            }
            if (this.fxT != null) {
                fW += e.a.a.b.b.a.h(2, this.fxT);
            }
            if (this.wbF != null) {
                fW += e.a.a.b.b.a.h(3, this.wbF);
            }
            if (this.pRd != null) {
                fW += e.a.a.b.b.a.h(4, this.pRd);
            }
            if (this.pQZ != null) {
                fW += e.a.a.b.b.a.h(5, this.pQZ);
            }
            fW += e.a.a.a.R(6, this.pSo);
            if (this.wbo != null) {
                fW += e.a.a.b.b.a.h(7, this.wbo);
            }
            return fW;
        } else if (i == 2) {
            e.a.a.a.a aVar2 = new e.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (fW = com.tencent.mm.bp.a.a(aVar2); fW > 0; fW = com.tencent.mm.bp.a.a(aVar2)) {
                if (!super.a(aVar2, this, fW)) {
                    aVar2.cKx();
                }
            }
            if (this.fxT == null) {
                throw new b("Not all required fields were included: req_key");
            } else if (this.wbF == null) {
                throw new b("Not all required fields were included: transfer_id");
            } else if (this.pRd == null) {
                throw new b("Not all required fields were included: transaction_id");
            } else if (this.pQZ != null) {
                return 0;
            } else {
                throw new b("Not all required fields were included: receiver_openid");
            }
        } else if (i != 3) {
            return -1;
        } else {
            e.a.a.a.a aVar3 = (e.a.a.a.a) objArr[0];
            mg mgVar = (mg) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            switch (intValue) {
                case 1:
                    LinkedList JD = aVar3.JD(intValue);
                    int size = JD.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        byte[] bArr = (byte[]) JD.get(intValue);
                        com.tencent.mm.bp.a fhVar = new fh();
                        e.a.a.a.a aVar4 = new e.a.a.a.a(bArr, unknownTagHandler);
                        for (boolean z = true; z; z = fhVar.a(aVar4, fhVar, com.tencent.mm.bp.a.a(aVar4))) {
                        }
                        mgVar.wQE = fhVar;
                    }
                    return 0;
                case 2:
                    mgVar.fxT = aVar3.AEQ.readString();
                    return 0;
                case 3:
                    mgVar.wbF = aVar3.AEQ.readString();
                    return 0;
                case 4:
                    mgVar.pRd = aVar3.AEQ.readString();
                    return 0;
                case 5:
                    mgVar.pQZ = aVar3.AEQ.readString();
                    return 0;
                case 6:
                    mgVar.pSo = aVar3.AEQ.rA();
                    return 0;
                case 7:
                    mgVar.wbo = aVar3.AEQ.readString();
                    return 0;
                default:
                    return -1;
            }
        }
    }
}
