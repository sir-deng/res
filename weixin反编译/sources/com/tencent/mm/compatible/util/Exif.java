...
public class Exif {
    ...
    public int parseFromFile(String str) {
        if (new File(str).exists()) {
            x.i(TAG, "parse EXIF from file on real fs, result: %d, path: %s", Integer.valueOf(nativeParseFile(str)), str);
    ...
    public int parseFromStream(InputStream inputStream) {
        ...
        x.i(TAG, "parse EXIF from Stream, ret = " + nativeParse);
        return nativeParse;
    }
    ...
    public static Exif fromFile(String str) {
        Exif exif = new Exif();
        try {
            x.i(TAG, "parse EXIF, result: " + exif.parseFromFile(str));
        } catch (IOException e) {
            x.w(TAG, "parse EXIF failed: " + e.getMessage());
        }
        return exif;
    }
    ...
    public static Exif fromStream(InputStream inputStream) {
        Exif exif = new Exif();
        try {
            x.i(TAG, "parse EXIF, result: " + exif.parseFromStream(inputStream));
        } catch (IOException e) {
            x.w(TAG, "parse EXIF failed: " + e.getMessage());
        }
        return exif;
