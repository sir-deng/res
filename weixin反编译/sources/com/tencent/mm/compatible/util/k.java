...
public final class k {
    private static String gJD = null;
    private static boolean gJE = false;
    private static HashMap<String, String> gJF = null;
    ...
    private static boolean a(String str, ClassLoader classLoader) {
        ...
        if (!gJE) {
            ...
            if (ShareTinkerInternals.oN(d)) {
                gJD = null;
            } else {
                File iA = SharePatchFileUtil.iA(tinkerApplicationLike.getApplication());
                d = SharePatchFileUtil.act(d);
                if (!(iA == null || d == null)) {
                    gJD = new File(iA.getAbsolutePath() + "/" + d).getAbsolutePath() + "/lib";
                }
            }
        }
        ...
        tinkerApplicationLike = MMApplicationLike.getTinkerApplicationLike();
        if (b.b(tinkerApplicationLike) && b.c(tinkerApplicationLike) && gJF != null) {
            for (String d2 : gJF.keySet()) {
                if (d2.equals(str2)) {
                    String str3 = gJD + "/" + d2;
                    File file = new File(str3);
                    if (!file.exists()) {
                        continue;
                    } else if (!tinkerApplicationLike.getTinkerLoadVerifyFlag() || SharePatchFileUtil.f(file, (String) gJF.get(d2))) {
                        try {
