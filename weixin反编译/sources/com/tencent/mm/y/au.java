...
public final class au {
    public static SharedPreferences Ht() {
        ...
        if (!sharedPreferences.getBoolean("key_auth_info_prefs_created", false)) {
            d.pVE.a(148, 12, 1, true);
            s sVar = new s(w.hbv + "autoauth.cfg");
            boolean commit;
            if (sVar.xuT || sVar.get(3) == null) {
                SharedPreferences sharedPreferences2 = ad.getContext().getSharedPreferences("auto_auth_key_prefs", 4);
                Editor edit = sharedPreferences.edit();
                if (bi.oN(sharedPreferences2.getString("_auth_key", ""))) {
                    x.i("MicroMsg.MMReqRespAuthComm", "summerauth auth_info_key_prefs not exit! neither autoauthcfg nor oldAAKsp existed just install! stack[%s]", bi.chl());
                } else {
