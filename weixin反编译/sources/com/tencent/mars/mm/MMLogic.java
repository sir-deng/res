...
public class MMLogic {
    ...
    private static void reportFlow(int i, int i2, int i3, int i4) {
        ...
        final b bVar = aa.VT().icK;
        x.i("C2Java", "reportNetFlow time[%d,%d] sum:%d wr[%d,%d] ws[%d,%d] mr[%d,%d] ms[%d,%d] fgbg:%b Moniter:%s", Long.valueOf(lastReportTime), Long.valueOf(bi.bz(lastReportTime)), Integer.valueOf(i5), Integer.valueOf(i), Integer.valueOf(totalWifiRecv), Integer.valueOf(i2), Integer.valueOf(totalWifiSend), Integer.valueOf(i3), Integer.valueOf(totalMobileRecv), Integer.valueOf(i4), Integer.valueOf(totalMobileSend), Boolean.valueOf(com.tencent.mm.sdk.a.b.foreground), bVar);
        if (bVar != null) {
            if (i5 >= 102400 || bi.bz(lastReportTime) >= 30) {
                lastReportTime = bi.Wx();
                    ...
                    aa.VV().post(new Runnable() {
                        public final void run() {
                            try {
                                ...
                                x.e("C2Java", "reportFlowData :%s", bi.i(th));
                            }
                        }
                    });
                } catch (Throwable th) {
    ...
    private static void onOOBNotify(final String str) {
        ...
        try {
            aa.VV().post(new Runnable() {
                public final void run() {
                    aa.VZ().onPush(268369923, bi.oN(str) ? null : str.getBytes());
                }
            });
        } catch (Throwable e) {
            x.e("C2Java", "onOOBNotify :%s", bi.i(e));
        }
    }
}
