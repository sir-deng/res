...
public class WakerLock {
    ...
    private static final boolean checkShouldLock() {
        if (bi.bA(lastChecktime) > 10000 || shouldLock == null) {
            boolean z = (m.eK(ad.getContext()) || m.eL(ad.getContext())) ? false : true;
            shouldLock = Boolean.valueOf(z);
            x.i(TAG, "checkShouldLock screen:%b chatging:%b res:%b checkTime:%d ", Boolean.valueOf(r3), Boolean.valueOf(r4), shouldLock, Long.valueOf(bi.bA(lastChecktime)));
            lastChecktime = bi.Wy();
        }
